...
int main(int argc, char *argv[])
{
    ...
    //Added by Reda: DEFINING PREREQUISTES FOR THE SELECT FUNCTION
    fd_set rfds, wfds;
    struct timeval time_out;
    FD_ZERO(&rfds);
    FD_ZERO(&wfds);
    FD_SET(server_desc_udp2, &rfds);
    FD_SET(server_desc_udp2, &wfds);
    time_out.tv_sec = 5;
    time_out.tv_usec = 0;
    while (1)
    {
        ...
        select(10, &rfds, NULL, NULL, &time_out); //added by Reda: waiting for the socket to be ready for reading.
        n = recvfrom(server_desc_udp2, (char *)buffer, RCVSIZE, MSG_WAITALL, (struct sockaddr *)&cliaddr, &len);
        buffer[n] = '\0';

        select(10, &rfds, NULL, NULL, &time_out);   
